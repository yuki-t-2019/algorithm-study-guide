...
/**
 * @brief Performs a depth-first search to determine
 *        if a subset of integers sums to a specified value.
 *
...
bool Dfs(
  ...
  if (elements.size() < 1 || elements.size() > kMaxElements) {
    throw std::invalid_argument(
      "The number of elements must be between 1 and "
  ...
  for (size_t i = 0; i < elements.size(); ++i) {
    if (elements[i] < kLowerBound || elements[i] > kUpperBound) {
      throw std::out_of_range(
        "Element at index " + std::to_string(i) + " is out of range.");
    }
  }
  ...
  if (Dfs(index + 1, current_sum, elements, target_sum))
    return true;
  if (Dfs(index + 1, current_sum + elements[index], elements, target_sum))
    return true;
